  ...
  <script type="text/javascript">
  	$(function(){
		...
		$(document).on('click','.view-research',function(e){
	   		...
	   		$.ajax({
	   			url: '<?= base_url()?>index.php/public/research/retrieve_data',
	   			type: 'POST',
	   			dataType: 'JSON',
				...
				<div class="row" style="padding-bottom: 30px;">
					...
					<div class="col-md-13 pr-0">
						<div class="activity-sched bg-white ptb-15">
							<h3 class="normal c-gray f-18 mb-15 bb pb-10">Capstone Research Studies</h3>
							<table class="research-table table" id="research-table">
								...
								<tbody>
									...
									<?php foreach($research as $key => $value):?>
										<tr>
											<td><a href="#" class="view-research" attr='<?= $value->caps_id ?>' style="text-decoration: none;"><?= $value->caps_id.". ". $value->title ?></a></td>
											<td style="text-align: center"><center><?= date("Y",strtotime($value->date_published))?></center></td>
											<td style="text-align: center"><center><a href="<?= base_url('plugins/images/research/capstone/'.$value->img_path)?>" target="_blank" download><?= $value->img_path !='' ? 'Download pdf':''?></a></center></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
