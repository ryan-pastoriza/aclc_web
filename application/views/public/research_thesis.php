 ...
 * @Last Modified time: 2019-04-21 14:33:01
 */
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <script type="text/javascript">
  	$(function(){
		...
		$(document).on('click','.view-research',function(e){
	   		...
	   		$.ajax({
	   			url: '<?= base_url()?>index.php/public/thesis/retrieve_data',
	   			type: 'POST',
	   			dataType: 'JSON',
					...
					<div class="col-md-5 pr-0">
						<div class="admission-leftpane bg-white">
							...
							<ul>
								...
								<li class="active"><a href="<?= base_url()?>index.php/public/thesis">Thesis</a><i class="fa fa-play"></i></li>
						...
						<div class="activity-sched bg-white ptb-15">
							...
							<table class="research-table table" id="research-table">
								...
								<tbody>
									...
									<?php foreach($thesis as $key => $value):?>
										<tr>
											<td><a href="#" class="view-research" attr='<?= $value->thesis_id ?>' style="text-decoration: none;"><?= $value->thesis_id.". ". $value->title ?></a></td>
											<td style="text-align: center"><center><?= date("Y",strtotime($value->date_published))?></center></td>
											<td style="text-align: center"><center><a href="<?= base_url('plugins/images/research/thesis/'.$value->img_path)?>" target="_blank" download><?= $value->img_path !='' ? 'Download pdf':''?></a></center></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
