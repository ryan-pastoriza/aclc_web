  ...
  <script type="text/javascript">
  	$(function(){
		...
		$(document).on('click','.view-research',function(e){
	   		...
	   		$.ajax({
	   			url: '<?= base_url()?>index.php/public/research_institutional/retrieve_data',
	   			type: 'POST',
	   			dataType: 'JSON',
				...
				<div class="row" style="padding-bottom: 30px;">
					...
					<div class="col-md-13 pr-0">
						<div class="activity-sched bg-white ptb-15">
							<h3 class="normal c-gray f-18 mb-15 bb pb-10">Institutional</h3>
							<table class="research-table table" id="research-table">
								<thead>
									<th class="col-xs-18" style="padding-left: 0">Institutional Title</th>
									<th class="col-xs-6" style="text-align: center">Year Published</th>
									<th class="col-xs-6" style="text-align: center">Download</th>
								</thead>
								<tbody>

									<?php foreach($institutional as $key => $value):?>
										<tr>
											<td><a href="#" class="view-research" attr='<?= $value->ins_id ?>' style="text-decoration: none;"><?= $value->ins_id.". ". $value->title ?></a></td>
											<td style="text-align: center"><center><?= date("Y",strtotime($value->date_published))?></center></td>
											<td style="text-align: center"><center><a href="<?= base_url('plugins/images/research/institutional/'.$value->img_path)?>" target="_blank" download><?= $value->img_path !='' ? 'Download pdf':''?></a></center></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
